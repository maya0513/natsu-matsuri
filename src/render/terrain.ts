...
// 台地⇄河川敷の境界帯は段差。石段(z∈[stairZ0,stairZ1])はなだらかな下り、それ以外は急斜面の崖で、
// どちらも歩いて下りられる（＝崖から落下できる）。丘・盛り土もどこでも歩いて登れる。
 ...
 * - x <  bankX: 河川敷（bankY）
 * - その間（境界帯）かつ 石段の z 範囲内: 台地の縁から河川敷へ段々に降下する石段
 * - その間（境界帯）かつ 石段の外: 台地の縁から河川敷へ落ちる急斜面の崖（smoothstep。歩いて下りられる）
 */
export const groundHeightAt = (x: number, z: number): number => {
  ...
  const topH = plateauHeight(WORLD.plateauX, z); // 台地の縁の高さ
  const t = (WORLD.plateauX - x) / (WORLD.plateauX - WORLD.bankX); // 0..1（東→西）
  if (z >= WORLD.stairZ0 && z <= WORLD.stairZ1) {
    const step = Math.ceil(t * WORLD.stairSteps); // 1..stairSteps（段々の下り）
    return topH + (WORLD.bankY - topH) * (step / WORLD.stairSteps);
  }
  // 石段の外は急斜面の崖。なだらかに見えて一気に落ちる S 字で河川敷へ降下する
  return topH + (WORLD.bankY - topH) * smooth(t);
};
